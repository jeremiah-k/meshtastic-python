...
PAUSE_AFTER_COMMAND: int = 2
PAUSE_AFTER_REBOOT: int = 7
INFO_READY_TIMEOUT_SECONDS: int = 60
INFO_READY_POLL_INTERVAL_SECONDS: int = 2
RESTORE_ATTEMPTS: int = 3
RESTORE_RETRY_DELAY_SECONDS: int = 2
...
def _restore_config_with_retries(config_path: Path) -> tuple[int, str]:
    ...
    output = ""
    result = 1
    deadline = time.monotonic() + RESTORE_TOTAL_TIMEOUT_SECONDS
    for attempt in range(RESTORE_ATTEMPTS):
        ...
        result, output = _run(
            ...
            "--configure",
            str(config_path),
            timeout=min(RESTORE_CONFIGURE_TIMEOUT_SECONDS, remaining),
        )
        if result == 0:
...
def test_smoke1_set_wifi_settings() -> None:
    ...
    assert re.search(r"^Set network\.wifi_psk\b", out, re.MULTILINE)
    assert "temp1234" not in out
    assert return_value == 0
