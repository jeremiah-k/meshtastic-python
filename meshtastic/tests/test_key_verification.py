...
def test_build_rejects_unknown_stage() -> None:
    ...
    with pytest.raises(ValueError, match="unknown key-verification stage"):
        buildKeyVerificationAdmin("revoke")
...
def test_build_later_stages_require_nonce() -> None:
    ...
    for stage in ("provide", "verify", "no-verify"):
        with pytest.raises(ValueError, match="requires the nonce"):
            buildKeyVerificationAdmin(stage)
...
def test_build_provide_requires_six_digit_security_number() -> None:
    ...
    with pytest.raises(ValueError, match="provide requires"):
        buildKeyVerificationAdmin("provide", nonce=5)
    with pytest.raises(ValueError, match="six digits"):
        buildKeyVerificationAdmin("provide", nonce=5, security_number=0)
    with pytest.raises(ValueError, match="six digits"):
        buildKeyVerificationAdmin("provide", nonce=5, security_number=1000000)
    message = buildKeyVerificationAdmin("provide", nonce=5, security_number=424242)
...
def test_build_rejects_out_of_range_inputs() -> None:
    ...
    with pytest.raises(ValueError, match="32-bit node number"):
        buildKeyVerificationAdmin("initiate", remote_nodenum=0x100000000)
    with pytest.raises(ValueError, match="unsigned 64-bit"):
        buildKeyVerificationAdmin("verify", nonce=-1)
    with pytest.raises(ValueError, match="unsigned 64-bit"):
        buildKeyVerificationAdmin("verify", nonce=0x10000000000000000)
...
def test_send_validates_timeout_and_my_info() -> None:
    ...
    request = buildKeyVerificationAdmin("initiate", remote_nodenum=7)
    with pytest.raises(ValueError, match="finite and positive"):
        sendKeyVerification(interface, request, timeout=0)
    missing = MagicMock(spec=MeshInterface)
    missing.myInfo = None
    with pytest.raises(RuntimeError, match="my_info"):
        sendKeyVerification(missing, request, timeout=1.0)
...
def test_send_targets_local_node_over_admin_app() -> None:
    ...
    request = buildKeyVerificationAdmin("initiate", remote_nodenum=0xABCD1234)
    with pytest.raises(TimeoutError):
        sendKeyVerification(interface, request, timeout=0.05)
...
def test_send_returns_matching_notification_from_pubsub() -> None:
    ...
    request = buildKeyVerificationAdmin("initiate", remote_nodenum=0xABCD1234)
    result = sendKeyVerification(interface, request, timeout=3.0)
...
def test_send_decision_stage_returns_without_notification(stage: str) -> None:
    ...
    interface = _interface_with_node_num(2478223698)
    request = buildKeyVerificationAdmin(stage, nonce=5)

    assert sendKeyVerification(interface, request, timeout=0.05) is None
...
def test_send_filters_stale_nonce_for_later_stages() -> None:
    ...
    interface.sendData.side_effect = _send
    request = buildKeyVerificationAdmin("provide", nonce=5, security_number=424242)
    with pytest.raises(TimeoutError):
        sendKeyVerification(interface, request, timeout=0.2)
...
def test_send_rejects_non_finite_timeout(timeout: float) -> None:
    ...
    with pytest.raises(ValueError, match="finite and positive"):
        sendKeyVerification(interface, request, timeout=timeout)
...
def test_send_ignores_expected_variant_when_nonce_extraction_fails(
    ...
    with pytest.raises(TimeoutError):
        sendKeyVerification(interface, request, timeout=0.02)
...
def test_concurrent_initiates_are_serialized_per_interface() -> None:
    ...
    interface.sendData.side_effect = _send
    first = buildKeyVerificationAdmin("initiate", remote_nodenum=0x11111111)
    second = buildKeyVerificationAdmin("initiate", remote_nodenum=0x22222222)

    def _run(name: str, request: admin_pb2.KeyVerificationAdmin) -> None:
        try:
            result = sendKeyVerification(interface, request, timeout=2.0)
            assert result is not None
            results[name] = result
