...
def test_mesh_interface_error_preserves_message_and_pickle_contract() -> None:
    ...
    )  # noqa: S301 - trusted local test object
...
def test_internal_exception_subclasses_preserve_public_mesh_interface_base() -> None:
    ...
    assert issubclass(
        StreamInterface.StreamInterfaceError, MeshInterface.MeshInterfaceError
    )
    assert issubclass(
        StreamInterface.PayloadTooLargeError, MeshInterface.MeshInterfaceError
    )
