...
from .discovery import DiscoveryManager, run_connected_device_lookup
from .errors import BLEError as BLEInterfaceError, BLEErrorHandler
...
class BLEInterface(MeshInterface):
    ...
    async def _with_timeout(awaitable, timeout: Optional[float], label: str):
        ...
        return await with_timeout(awaitable, timeout, label)
    ...
    def _find_connected_devices(self, address: Optional[str]) -> List[BLEDevice]:
        ...
        discovery = getattr(self, "_discovery_manager", None)
        strategy = getattr(discovery, "connected_strategy", None)
        if strategy is None:
            strategy = DiscoveryManager().connected_strategy
        return run_connected_device_lookup(strategy, address)
    ...
    def _wait_for_disconnect_notifications(
        ...
        flush_event = Event()
        pub_thread = _get_publishing_thread()
        self.error_handler.safe_execute(
            lambda: pub_thread.queueWork(flush_event.set),
        ...
        if not flush_event.wait(timeout=timeout):
            thread = getattr(pub_thread, "thread", None)
    ...
    def _drain_publish_queue(self, flush_event: Event) -> None:
        ...
        """
        pub_thread = _get_publishing_thread()
        queue = getattr(pub_thread, "queue", None)
        if queue is None:
            return
...
def _bleak_supports_connected_fallback() -> bool:
    """Backward-compatible wrapper for older monkeypatch sites."""
    return bleak_supports_connected_fallback()


def _get_publishing_thread():
    """Return the publishingThread object, honoring shim monkeypatches."""
    ble_module = sys.modules.get("meshtastic.ble_interface")
    if ble_module and hasattr(ble_module, "publishingThread"):
        return ble_module.publishingThread
